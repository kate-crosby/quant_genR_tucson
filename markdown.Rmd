...
Since we have the narrow sense heritability (h^2^), and we have the phenotypic variances (V_p_), we can easily calculate the additive variance (V_a_) of each of the trait, by re-arranging the equation h^2^ = V_a_/V_p_ to V_a_= h^2^ * V_p_. Note that the function `{r}marker_h2` calculates both as well, we are merely going through the motions here to understand Gmatrix construction. We can also use the h^2^ in assessing the response to various selection coefficients.
